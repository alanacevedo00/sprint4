...
def filtrar_cheques(cheques, dni, tipo_cheque, estado=None, fecha_inicio=None, fecha_fin=None):
    resultados = []
    cheques_repetidos = set()

    for cheque in cheques:
        nro_cheque = cheque["NroCheque"]
        if (dni, nro_cheque) in cheques_repetidos:
            print(f"Error: Número de cheque repetido {nro_cheque} para el DNI {dni}.")
            continue
        cheques_repetidos.add((dni, nro_cheque))

        if cheque["DNI"] == dni and (not estado or cheque["Estado"] == estado):
            fecha_key = "FechaOrigen" if tipo_cheque == "EMITIDO" else "FechaPago"
            if fecha_key in cheque:
                fecha_cheque = datetime.fromtimestamp(int(cheque[fecha_key]))
            else:
                continue
            ...
            if (not fecha_inicio or fecha_inicio <= fecha_cheque) and (not fecha_fin or fecha_cheque <= fecha_fin):
                ...
                print(f"No se cumplió la condición para el cheque: {cheque}")
...
def main():
    ...
    try:
        ...
        print(f"Cheques filtrados: {cheques_filtrados}")  
